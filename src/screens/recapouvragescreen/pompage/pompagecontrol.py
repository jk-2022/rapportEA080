...
class PompageControl(ft.Card):
    def __init__(self, state,  formcontrol):
        ...
        self.delete_btn=ft.Button('Supprimer',icon=ft.Icons.DELETE,icon_color=ft.Colors.RED,on_click=lambda e :self.showDelete())
        self.delete_btn.visible=False

        self.content=ft.Container(
            border_radius=10,
            border=ft.Border.all(1,ft.Colors.PINK_500),
            # expand=True,
            content=ft.Column(
                [
                    ft.Row(
                        [
                            ft.Text("Pompage",text_align=ft.TextAlign.CENTER,color=ft.Colors.PINK_500)
    ...
    def updateData(self):
        ...
        donnees=load_one_pompage(ouvrage.id)
        self.donnees=donnees #Important pour stocker les donnees en cas d'update
