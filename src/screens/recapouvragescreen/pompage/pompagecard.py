...
def PompageCard(pompe,showUpdateData,showDelete,shareData):
    pompe_cnt=ft.Column(
        expand=True,
        controls=[]
    )
    
    list_item=['id','ouvrage_id','localisation_id','created_at']
    for key, val in pompe[0].items():
        if key in list_item or val=="" or val==None:
            pass 
        else:
            pompe_cnt.controls.append(
        ...
        content=ft.Column(
            ...
            controls=[
                ...
                ft.Container(
                    ...
                    border=ft.Border.only(left=ft.BorderSide(width=4, color=ft.Colors.ORANGE_500)),
                    content=pompe_cnt
                ),
                ft.Row(
                    [
